...
function confidenceByDate(multi) {
  ...
  for (let i = 0; i < time.length; i++) {
    const gusts = gustKeys.map(k => src[k] && src[k][i]).filter(x => x != null);
    const rains = rainKeys.map(k => src[k] && src[k][i]).filter(x => x != null);
    if (!gusts.length) continue;
    const gSpread = Math.max(...gusts) - Math.min(...gusts);
    const rainDisagree = (Math.max(...rains) - Math.min(...rains)) > 3; // mm
