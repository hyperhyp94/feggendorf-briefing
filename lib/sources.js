...
export async function fetchOpenMeteoMulti(lat, lon) {
  try {
    const url = `https://api.open-meteo.com/v1/forecast?latitude=${lat}&longitude=${lon}`
      + `&daily=wind_gusts_10m_max,precipitation_sum,wind_speed_10m_max,wind_direction_10m_dominant&wind_speed_unit=kmh`
      + `&models=icon_seamless,ecmwf_ifs025,gfs_seamless&timezone=Europe%2FBerlin&forecast_days=10`;
    return { ok: true, data: await getJSON(url) };
  } catch (e) { return { ok: false, error: String(e) }; }
}

// ---- Open-Meteo multi-model hourly: stündliche Wind-/Temp-Daten für ICON/ECMWF/GFS ----
const HOURLY_MULTI = [
  'wind_speed_10m', 'wind_direction_10m', 'wind_gusts_10m',
  'temperature_2m', 'precipitation_probability'
].join(',');

export async function fetchOpenMeteoMultiHourly(lat, lon) {
  try {
    const url = `https://api.open-meteo.com/v1/forecast?latitude=${lat}&longitude=${lon}`
      + `&hourly=${HOURLY_MULTI}&wind_speed_unit=kmh`
      + `&models=icon_seamless,ecmwf_ifs025,gfs_seamless&timezone=Europe%2FBerlin&forecast_days=3`;
    return { ok: true, data: await getJSON(url) };
  } catch (e) { return { ok: false, error: String(e) }; }
}
...
// ---- Convek: soaring-specific forecast (thermal strength, cloudbase, day rating) ----
export async function fetchConvek(key, lat, lon) {
  try {
    if (!key) return { ok: false, error: 'no CONVEK_API_KEY set' };
    const url = `https://api.convek.dev/v1/site?lat=${lat}&lon=${lon}&region=de&resolution=4km`;
    const r = await fetch(url, { headers: { ...UA.headers, 'Authorization': `Bearer ${key}` } });
...
// ---- DWD direct: 10-day station forecast (temp, pressure, humidity via WarnWetter API) ----
export async function fetchDwdStation(stationId) {
  try {
    if (!stationId) return { ok: false, error: 'no DWD_STATION_ID set' };
    const url = `https://app-prod-ws.warnwetter.de/v30/stationOverviewExtended?stationIds=${stationId}`;
